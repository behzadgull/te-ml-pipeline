# teMatDb 3-stratum scoring (a0/a/b) re-run against File A: refits the 4
# frozen-hyperparameter models and recomputes Duan smear factors against
# File A (load_target_data monkeypatched, no src/ edit), then reuses the
# already-featurized data/external/tematdb/_scoring_df.csv unchanged (C1/C2
# confirmed strata membership doesn't move between snapshots). NOTE:
# bootstrap_r2_ci here uses N_BOOT=2000, vs n_boot=1000 in the original
# tematdb_scoring.py -- the sole source of this file's CI-bound deltas
# against the original when compared via the control run (see
# scripts/README.md). Produced
# results/20260911T114356_tematdb_scoring_fileA/.
...
NEW_RESULTS_DIR = "results/20260911T114356_tematdb_scoring_fileA"
RNG_SEED = 0
N_BOOT = 2000

# Same PF2 mechanism as B: rebind load_target_data inside external_validation's
# own namespace so fit_frozen_external_model()/compute_training_smear_factors()
# load File A. Process-local, no src/ edit, no file touched.
ev.load_target_data = functools.partial(ev.load_target_data, processed_data_dir=FILE_A_DIR)

print("=== D1: teMatDb 3-stratum scoring, re-run against File A ===", flush=True)
print("Reusing the cached featurized teMatDb scoring dataframe (data/external/tematdb/_scoring_df.csv) --", flush=True)
print("justified because C1/C2 confirmed a0/a/parseable/stratum-b membership is IDENTICAL between File A and File B", flush=True)
print("(teMatDb-side featurization does not depend on the training dataset at all).", flush=True)
...
print("\n=== S1: refit all four frozen models on 100% of File A training ===", flush=True)
models = {}
training_rows_used = {}
for target in ev.TARGETS:
    model, feature_cols_train, n_rows = ev.fit_frozen_external_model(target, device="cpu")
    models[target] = model
    training_rows_used[target] = n_rows
    print(f"  {target}: refit on {n_rows:,} training rows", flush=True)

print("\n=== S1: recompute + freeze Duan smear factors (File A training OOF only) ===", flush=True)
...
metrics_out = {
    ...
    "note": "Re-run against File A per CLAUDE.md's Canonical Dataset section (commit e1ede95). "
            "Original run (results/20260910T123047_tematdb_external/) used File B and is preserved unchanged. "
            "teMatDb-side scoring dataframe (data/external/tematdb/_scoring_df.csv) reused unchanged since C1/C2 "
            "confirmed strata membership is identical between File A and File B.",
...
print("TEMATDB_D_FILEA_DONE", flush=True)
